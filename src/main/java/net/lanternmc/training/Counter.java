...
import net.lanternmc.training.utils.SoundMachine;
...
public class Counter {
    ...
    public void addLogBlock(Block block) {
        allBlock.add(block);
        Training.getPlacedBlocks().put(block, block.getState().getData());
    }
    ...
    public void instantBreakBlock() {
        for (Block b : allBlock) {
            Utils.breakBlock(b);
            Training.getPlacedBlocks().remove(b);
        }
        allBlock.clear();
    }
    ...
    public void setCheckPoint(Location loc) {
        LocationManager.setPlayCheckPoint(player, loc);
        Block target = LocationManager.getPlayerCheckpoint(player).add(0, -1, 0).getBlock().getRelative(BlockFace.DOWN, 3);
    ...
    public void teleportCheckPoint() {
        player.teleport(LocationManager.getPlayerCheckpoint(player));
        Block target = LocationManager.getPlayerCheckpoint(player).getBlock().getRelative(BlockFace.DOWN, 3);
        if (target.getType() == Material.CHEST) {
            Training.clearInventory(player);
            Chest chest = (Chest) target.getState();
            for (ItemStack stack : chest.getBlockInventory().getContents())
    ...
    public class BreakRunnable implements Runnable {
        ...
        public BreakRunnable(ArrayList<Block> allBlocks) {
            ...
            task = Bukkit.getScheduler().runTaskTimer(Training.getInstance(), this, 10, tick);
        }
